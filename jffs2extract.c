...
void putblock(char *b, size_t bsize, size_t * rsize,
    ...
    switch (n->compr) {
        ...
        case JFFS2_COMPR_LZMA:
            dlen = bsize;
            jffs2_lzma_decompress(  (Bytef *) ((char *) n) + sizeof (struct jffs2_raw_inode),
                                    (Bytef *) b + je32_to_cpu(n->offset),
                                    (uint32_t) je32_to_cpu(n->csize),
                                    (uint32_t*) &dlen);
            break;

        case JFFS2_COMPR_LZO:
            dlen = bsize;
            lzo1x_decompress_safe(  (lzo_bytep) ((char *) n) + sizeof (struct jffs2_raw_inode),
                                    (lzo_uint) je32_to_cpu(n->csize),
                                    (lzo_bytep) b + je32_to_cpu(n->offset),
                                    (lzo_uintp) &dlen,NULL);
            break;
    ...
    *rsize = je32_to_cpu(n->dsize); //wangyaoyang : from n->isize to n->dsize
}
...
int main(int argc, char **argv) {
    ...
    memset (target_dir,0x00,128);
    if (argc < 2) {
        usage(argv);
    }

    while ((opt = getopt(argc, argv, "hf:C:txvlb")) > 0) {
        switch (opt) {
            case 'h':
                usage(argv);
                break;
            case 'f':
                ...
                break;
            case 'C':
                strncpy (target_dir,optarg,sizeof(target_dir));
                break;
            case 't':
    ...
    if (dump && imgfile) {
        static char    dest_file[128];
        memset (dest_file,0x00,128);
        snprintf(dest_file,sizeof(dest_file)-1,"%s.%s",imgfile,(dump == 1) ? "l" : "b");
        printf("\nConverting source file '%s' to %s format './%s'...\n",
                imgfile,(dump == 1) ? "Little endian" : "Big endian",dest_file);
        if (dump_main((dump == 1) ? __LITTLE_ENDIAN : __BIG_ENDIAN,dest_file,imgfile))
            exit (EXIT_FAILURE);
        imgfile = dest_file;
    }
    
    if (v && imgfile && (fd = open(imgfile, O_RDONLY)) >= 0) {
        ...
        if (strlen(target_dir) <= 0) {
            strcpy(target_dir, "jffs2");
        }
        mkdir(target_dir,0777);
        int ret = chdir((const char*)target_dir);
        if (ret == 0) printf("\nExtracting Jffs2 file system into ./%s\n\n",target_dir);
        if (argc > optind) {
            int i;
        ...
        free(buf);
    } else if (fd < 0) sys_errmsg_die("%s", argv[optind]);
