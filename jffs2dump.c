...
static void set_options(int endian, char* dest_file, char* src_file) {
    target_endian = endian;
    convertendian = 1;
    recalccrc = 1;
    strcpy(cnvfile, dest_file);
    img = src_file;
}
...
void do_dumpcontent(void) {
    ...
    while (p < (data + imglen)) {
        ...
        node->u.nodetype = ((jint16_t){t16(type)});
    ...
    while (p < (data + imglen)) {
        ...
        switch (je16_to_cpu(node->u.nodetype)) {
            ...
            case JFFS2_NODETYPE_INODE:
                ...
                written = write(fd, &newnode, sizeof (struct jffs2_raw_inode));
                written = write(fd, p + sizeof (struct jffs2_raw_inode), PAD(je32_to_cpu(node->i.totlen) - sizeof (struct jffs2_raw_inode)));
            ...
            case JFFS2_NODETYPE_DIRENT:
                ...
                written = write(fd, &newnode, sizeof (struct jffs2_raw_dirent));
                written = write(fd, p + sizeof (struct jffs2_raw_dirent), PAD(je32_to_cpu(node->d.totlen) - sizeof (struct jffs2_raw_dirent)));
                p += PAD(je32_to_cpu(node->d.totlen));
                break;
            ...
            case JFFS2_NODETYPE_PADDING:
                ...
                written = write(fd, &newnode, sizeof (struct jffs2_unknown_node));
                len = PAD(je32_to_cpu(node->u.totlen) - sizeof (struct jffs2_unknown_node));
                if (len > 0)
                    written = write(fd, p + sizeof (struct jffs2_unknown_node), len);
            ...
            case JFFS2_NODETYPE_SUMMARY:
            {
                ...
                written = write(fd, &newnode, sizeof (struct jffs2_raw_summary));
                // write out new summary data
                written = write(fd, &node->s.sum, sum_len + sizeof (struct jffs2_sum_marker));
...
int dump_main(int endian, char* dest_file, char* src_file) {
    int fs;
    ssize_t beenRead = 0;

    set_options(endian, dest_file, src_file); //__LITTLE_ENDIAN;
    /* Open the input file */
    if ((fs = open(img, O_RDONLY)) == -1) {
        perror("open input file");
        return (1);
    }

    // get image length
    imglen = lseek(fs, 0, SEEK_END);
    lseek(fs, 0, SEEK_SET);
